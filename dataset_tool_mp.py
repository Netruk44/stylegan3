...
@click.option('--multiproc', help='Use multiple processes to create the dataset', type=int, default=1)
def convert_dataset(
    ctx: click.Context,
    source: str,
    dest: str,
    ...
    if dest.endswith('.zip'):
        ctx.fail('Cannot create zip archives with multi proc dataset tool')
    ...
    # Check if dest already exists. If not, create it. If it does, ensure it's empty.
    if os.path.isdir(dest):
        if os.listdir(dest):
            ctx.fail('--dest output directory must be empty')
    else:
        os.makedirs(dest)
    
    # Phase 1, preprocess files into destination directory.
    with Manager() as manager:
        ...
        # Tack on to input_iter dicts
        if resolution is None: resolution = (None, None)
        input_iter = map(lambda x: dict(
            x, 
            dest=dest, 
            transform=transform, 
    ...
    if all(x[1] is not None for x in label_mapping):
        ...
        print("WARN: Not all images had labels. Skipping dataset.json.")
